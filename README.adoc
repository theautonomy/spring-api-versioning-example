...
NOTE: This example demonstrates header-based versioning. Spring Framework 7.0 also supports other versioning strategies including path-based (e.g., `/v1/users`), query parameter-based (e.g., `?version=1.0`), and media type-based (e.g., `Accept: application/vnd.example.v1+json`). See the link:https://docs.spring.io/spring-framework/reference/web/webmvc-versioning.html#mvc-ann-apiversion-resolution[Spring Framework documentation] for more details on all available strategies.
...
Spring Framework 7.0 supports multiple versioning resolution strategies. To use a different strategy, modify the `WebMvcConfig` configuration:

[source,java]
----
// Path-based versioning (e.g., /v1/api/users)
configurer.usePathPrefix("/v{version}");

// Query parameter versioning (e.g., /api/users?version=1.0)
...
You can also combine multiple strategies for flexibility. See the link:https://docs.spring.io/spring-framework/reference/web/webmvc-versioning.html#mvc-ann-apiversion-resolution[API Version Resolution documentation] for complete details.
